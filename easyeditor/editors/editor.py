...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            # 如果开启了模型并行，就让框架自动分配显卡；否则不自动分配（默认加载到 cuda:0 ）
            device_map = 'auto' if hparams.model_parallel else None
            # 如果配置里开启了 fp16，就用半精度；否则默认用全精度。
            # 先检查配置里有没有fp16这个选项
            # 再看它是否为True
            ...
            if 't5' in self.model_name.lower():
                ...
                # 加载一个因果语言模型，专门用于文本生成（续写、对话、回答）
                # trust_remote_code=True，非常重要，允许执行模型仓库里的自定义代码，不加这个，很多国产大模型（如 Qwen、ChatGLM）会加载失败
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, **model_kwargs, trust_remote_code=True)
                # 加载与模型配套的【分词器】（Tokenizer） 把文字 → 模型能看懂的数字（token）
                self.tok = AutoTokenizer.from_pretrained(self.model_name,trust_remote_code=True)
                # 给分词器设置【填充标记】，模型要求输入的句子长度必须一样
                self.tok.pad_token_id = self.tok.eos_token_id
    ...
    def edit(self,
            ...
            #  rephrase_prompts: Optional[Union[str, List[str]]] = None,
             ...
             verbose=True,
             **kwargs  # 剩下的（如 subject, keep_original_weight）
             ):
        """
        `prompts`: list or str
            the prompts to edit

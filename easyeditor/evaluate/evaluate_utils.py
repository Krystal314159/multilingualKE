...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False):
    print('***************vanilla_generation:', vanilla_generation)
    # vanilla_generation这个是针对grace算法的
...
# 我们走的是这个分支
    ...
    # 计算：prompt 长度 + 填充长度  得到：每一句话从哪里开始是真正的答案。
    ...
    # 不计算梯度，把 prompt+target 输入模型，得到输出 logits（模型对每个 token 的预测）
    with torch.no_grad():
        ...
        if locality:
            ...
            if isinstance(answers, list):
                if len(answers) == 0:
                    return [[]]
                if isinstance(answers[0], list) and len(answers[0]) == 0:
                    # 如果切片结果为空，返回完整的预测结果
                    full_answers = torch.argmax(logits, dim=-1).squeeze().detach().cpu().numpy().tolist()
                    if isinstance(full_answers, list):
                        return [full_answers]
                    else:
                        return [[full_answers]]
            return answers if type(answers[0]) is list else [answers,]
